...
class DenseNetFinetune(nn.Module):
    ...
    # adaptable for size other than 224
    def forward(self, x):
        features = self.net.features(x)
        out = F.relu(features, inplace=True)
        out = F.avg_pool2d(out, kernel_size=features.size(2), stride=1).view(features.size(0), -1)
...
class DenseAttention(nn.Module):
    ...
    def __init__(self, num_classes, net_cls=densenet201):
...
densenet201_finetune = partial(DenseNetFinetune, net_cls=densenet201)
